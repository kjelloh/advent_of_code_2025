...
From the getgo I transformed the problem into a modulus 'clock arithmentic' one. Meaning I applied the 'stepping' plus or minus and then 'wrapped' the result into the 0..99 range with the modulus operation.
...
But whatching some youtube videos after I solved it calmed me down. The ones I watched encountered the same bewilderment and confusion. It seems this was in fact a nice twist on the 'part one' puzzle!

I ended up solving it kind-of 'brute force' with each corner case taken care of in an if-else chain of statements.

First I transformed the solution to use a 'current' and 'next' position. Where the 'next' position was the value before wrapping it to the nob values. 
...
Finally, if we end up below zero, we pass zero at least once (as we start from within the 'above zero' nob-range 0..99). Then we pass zero once for each interval of 100 we travel. But - We need to check for the corner case where we started at zero. In this case we reduce the number of clicks with one that we have already counted when we ended up there after previous turn.

I am still a bit weary about this solution. For one it feels a bit fragile and convolutet. Secondly I kind-of feel we maybe should have operated on the actual distance traveled (and not only on the next position)? Right now I will go with the result that this solution produces the right answer for all inputs, and leave it at that?
